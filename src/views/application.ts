...
export class ApplicationViewDataProvider extends BaseTreeViewDataProvider {
    ...
    if (element) {
      if (element.application) {
        ...
        return await this.loadNumbers(
          (element as ApplicationTreeItem).application.id,
        );
      }
      return [];
    }
  ...
  async linkApplication(node: ApplicationTreeItem): Promise<void> {
    await openUrl(
      `https://dashboard.nexmo.com/applications/${node.application.id}`,
    );
  }
  ...
  async createApplication(): Promise<void> {
    ...
    if (createResult) {
      ...
      vscode.window.showInformationMessage(
        `Successfully created application "${state.name}".`,
      );
    }
  }
  ...
  async updateApplication(node: ApplicationTreeItem): Promise<void> {
    ...
    if (updateResult) {
      this.refresh();
      vscode.window.showInformationMessage(
        `Successfully updated application "${state.name}".`,
      );
    }
  }
  ...
  async deleteApplication(node: ApplicationTreeItem): Promise<void> {
    ...
    if (result === true) {
      ...
      if (deleteResult) {
        this.refresh();
        vscode.window.showInformationMessage(
          `Successfully deleted application "${node.label}".`,
        );
      }
    } else {
      return;
  ...
  async addVoice(node: ApplicationTreeItem): Promise<void> {
    const state = await VoiceCapabilityFlow.collectInputs(
      `Add Voice Capability to application ${node.label}`,
    );
    ...
    if (updateResult) {
      this.refresh();
      vscode.window.showInformationMessage(
        `Successfully added voice to application "${application.name}".`,
      );
    }
  }
  ...
  async updateVoice(node: ApplicationTreeItem): Promise<void> {
    const state = await VoiceCapabilityFlow.collectInputs(
      `Update Voice Capability for application ${node.label}`,
      node,
    );
    ...
    if (updateResult) {
      this.refresh();
      vscode.window.showInformationMessage(
        `Successfully update voice to application "${application.name}".`,
      );
    }
  }
  ...
  async deleteVoice(node: ApplicationTreeItem): Promise<void> {
    ...
    if (result === true) {
      ...
      if (deleteResult) {
        this.refresh();
        vscode.window.showInformationMessage(
          `Successfully disabled voice on application "${node.label}".`,
        );
      }
    } else {
      return;
  ...
  async addRTC(node: ApplicationTreeItem): Promise<void> {
    const state = await RTCCapabilityFlow.collectInputs(
      `Add RTC Capability to application ${node.label}`,
    );
    ...
    if (updateResult) {
      this.refresh();
      vscode.window.showInformationMessage(
        `Successfully added RTC to application "${application.name}".`,
      );
    }
  }
  ...
  async updateRTC(node: ApplicationTreeItem): Promise<void> {
    const state = await RTCCapabilityFlow.collectInputs(
      `Update RTC Capability for application ${node.label}`,
      node,
    );
    ...
    if (updateResult) {
      this.refresh();
      vscode.window.showInformationMessage(
        `Successfully update RTC to application "${application.name}".`,
      );
    }
  }
  ...
  async deleteRTC(node: ApplicationTreeItem): Promise<void> {
    ...
    if (result === true) {
      ...
      if (deleteResult) {
        this.refresh();
        vscode.window.showInformationMessage(
          `Successfully disabled RTC on application "${node.label}".`,
        );
      }
    } else {
      return;
  ...
  async addMessages(node: ApplicationTreeItem): Promise<void> {
    const state = await MessageCapabilityFlow.collectInputs(
      `Add Messages Capability to application ${node.label}`,
    );
    ...
    if (updateResult) {
      this.refresh();
      vscode.window.showInformationMessage(
        `Successfully added Messages to application "${application.name}".`,
      );
    }
  }
  ...
  async updateMessages(node: ApplicationTreeItem): Promise<void> {
    const state = await MessageCapabilityFlow.collectInputs(
      `Update Messages Capability on application ${node.label}`,
    );
    ...
    if (updateResult) {
      this.refresh();
      vscode.window.showInformationMessage(
        `Successfully update Messages to application "${application.name}".`,
      );
    }
  }
  ...
  async deleteMessages(node: ApplicationTreeItem): Promise<void> {
    ...
    if (result === true) {
      ...
      if (deleteResult) {
        this.refresh();
        vscode.window.showInformationMessage(
          `Successfully disabled Messages on application "${node.label}".`,
        );
      }
    } else {
      return;
  ...
  async addVBC(node: ApplicationTreeItem): Promise<void> {
    ...
    if (updateResult) {
      this.refresh();
      vscode.window.showInformationMessage(
        `Successfully added VBC to application "${application.name}".`,
      );
    }
  }
  ...
  async deleteVBC(node: ApplicationTreeItem): Promise<void> {
    ...
    if (result === true) {
      ...
      if (deleteResult) {
        this.refresh();
        vscode.window.showInformationMessage(
          `Successfully disabled VBC on application "${node.label}".`,
        );
      }
    } else {
      return;
