...
export abstract class CreateApplicationFlow {
  public static async collectInputs(): Promise<CreateApplicationState> {
    ...
    await MultiStepInput.run((input) =>
      this.inputApplicationName(input, state),
    );
    return state as CreateApplicationState;
  }

  private static async inputApplicationName(
    input: MultiStepInput,
    state: Partial<CreateApplicationState>,
  ) {
  ...
  private static async inputPublicKey(
    input: MultiStepInput,
    state: Partial<CreateApplicationState>,
  ) {
    state.publicKey = await input.showInputBox({
      title,
      step: 2,
      totalSteps: 2,
      value: typeof state.publicKey === "string" ? state.publicKey : "",
