...
export abstract class VoiceCapabilityFlow {
  ...
  public static async collectInputs(
    ...
    await MultiStepInput.run((input) =>
      this.inputAnswerUrlAddress(input, state),
    );
    return state as VoiceCapabilityState;
  }

  private static async inputAnswerUrlAddress(
  ...
  private static async inputAnswerHttpMethod(
    input: MultiStepInput,
    state: Partial<VoiceCapabilityState>,
  ) {
  ...
  private static async inputFallbackUrlAddress(
    input: MultiStepInput,
    state: Partial<VoiceCapabilityState>,
  ) {
    ...
    return (input: MultiStepInput) =>
      this.inputFallbackHttpMethod(input, state);
  }

  private static async inputFallbackHttpMethod(
    input: MultiStepInput,
    state: Partial<VoiceCapabilityState>,
  ) {
    const result = await input.showQuickPick({
